...
- __coltake__ = Object created in the for-loop lateron, is only needed if you want only a part of the iterations/chains in your result. It simply is a vector of values $\in$ [1, iterations*chains] for every iteration (of chains) that should be used.
...
owncount(col_result, "VIF", 10, "max") %>%
    ...
    select(Predictor, Rsquared, VIF, count) %>% 
    xtable(.,
    caption = "Collinearity statistics of the used predictors. VIF = 1/R² = Variance inflation factor."
) %>%
    print(file = "./4_output/Plots and tables/Collinearity.txt", caption.placement = getOption("xtable.caption.placement", "top"), include.rownames=FALSE)
...
# Mean Importance in percent over all chains and iterations
ggplot(tree_vars_imp_mean_EU, aes(y = names, x = mean), size = 2) +
    geom_linerange(aes(xmin = mean - sd, xmax = mean + sd), col = "#1e854b", linewidth = 1.2) +
    geom_point(col = "black", size = 3) +
    xlab("Permutation importance in %") +
    ylab("Predictors in BART component") +
...
# Boxplot = Importance of that variable over the CV-runs
# Line = Exact variable importance per iteration
ggplot(my_dataframe_EU %>% filter(variable == "mean"), aes(x = L2, y = value, group = name)) +
    geom_line(aes(col = name)) +
    geom_boxplot(aes(col = name), fill = NA) + 
        xlab("CV run") +
    ylab("Permutation importance in %") +
...
ggplot(tree_vars_imp_mean_WORLD, aes(y = names, x = mean), size = 2) +
    geom_linerange(aes(xmin = mean - sd, xmax = mean + sd), col = "#1e854b", linewidth = 1.2) +
    geom_point(col = "black", size = 3) +
    xlab("Permutation importance in %") +
    ylab("Predictors in BART component") +
...
# Boxplot = Importance of that variable over the CV-runs
# Line = Exact variable importance per iteration
ggplot(my_dataframe_WORLD %>% filter(variable == "mean"), aes(x = L2, y = value, group = name)) +
    geom_line(aes(col = name)) +
    geom_boxplot(aes(col = name), fill = NA) + 
        xlab("CV run") +
    ylab("Permutation importance in %") +
...
ggplot(tree_vars_imp_mean_TOGETHER, aes(y = names, x = mean), size = 2) +
    geom_linerange(aes(xmin = mean - sd, xmax = mean + sd), col = "#1e854b", linewidth = 1.2) +
    geom_point(col = "black", size = 3) +
    xlab("Permutation importance in %") +
    ylab("Predictors in BART component") +
...
print(file = "./4_output/Plots and tables/WORLD_Test_Statistics.txt", caption.placement = getOption("xtable.caption.placement", "top"))
```
...
test_stat3 <- c(0.9, 0.9, 0.9, NA, 1,0, 0.05) %>% 
    rbind(test_stat[26,]) %>% 
    rbind(test_stat2[26,])

rownames(test_stat3) <- c("Min. Thresh", "Europe", "World")
...
print(file = "./4_output/Plots and tables/MEAN_Test_Statistics.txt", caption.placement = getOption("xtable.caption.placement", "top"))
...
#### Base - Europe
Remake of the plots in Mapsf to try different styles
...
final_res_curr <- rast("./4_output/Result/TODAY_Final_map.tif")
final_res_fut <- rast("./4_output/Result/FUTURE_Final_map.tif")
my_bbox <- matrix(c(-1275875, 4470206, 3696405, 7957273), byrow = F, nrow = 2) %>% bbox() %>% extent()
...
inset <- c(-0.035, -0.02, 0, 0)
inset <- c(0, 0, 0, 0)
{
layout.show(6)
plot(final_res_curr[["final"]], ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(AOI$geometry, add = T)
legend("topleft", legend = "A) Current suitability", cex = 2, box.lty = 0, bg = NA, inset = inset)
...
plot(c(0,2),c(0,1),type = 'n', axes = F,xlab = '', ylab = '', main = TeX("$\\textbf{Standard\\ deviation}$"), cex.main = 2)
text(x=0.5, y = seq(0.1,0.85,l=4), labels = seq(minmax_curr[1],minmax_curr[2],l=4) %>% round(2), cex = 2)
rasterImage(legend_image, 0, 0, 0.2,1)

text(x=1.7, y = seq(0.1,0.85,l=4), labels = seq(minmax_fut[1],minmax_fut[2],l=4) %>% round(2), cex = 2)
...
plot(final_res_curr[["std"]], ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(AOI$geometry, add = T)
legend("topleft", legend = "B) Std. of A", cex = 2, box.lty = 0, bg = NA, inset = inset)
plot(final_res_fut[["final"]], ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(AOI$geometry, add = T)
legend("topleft", legend = "C) Future suitability", cex = 2, box.lty = 0, bg = NA, inset = inset)
plot(final_res_fut[["std"]], ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(AOI$geometry, add = T)
legend("topleft", legend = "D) Std. of C", cex = 2, box.lty = 0, bg = NA, inset = inset)
}
```
...
{
layout.show(6)
plot(final_res_curr[["final"]], ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(DE_districts$geometry[DE_districts$GF != 8], add = T)
legend("topright", legend = "A) Current suitability", cex = 2, box.lty = 0, bg = NA, inset = inset)
...
plot(final_res_curr[["std"]], ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(DE_districts$geometry[DE_districts$GF != 8], add = T)
legend("topright", legend = "B) Std. of A", cex = 2, box.lty = 0, bg = NA, inset = inset)
plot(final_res_fut[["final"]], ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(DE_districts$geometry[DE_districts$GF != 8], add = T)
legend("topright", legend = "C) Future suitability", cex = 2, box.lty = 0, bg = NA, inset = inset)
plot(final_res_fut[["std"]], ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(DE_districts$geometry[DE_districts$GF != 8], add = T)
legend("topright", legend = "D) Std. of C", cex = 2, box.lty = 0, bg = NA, inset = inset)
}
```
...
{
layout.show(4)
plot(wolf_overlap, ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(DE_districts$geometry[DE_districts$GF != 8], add = T)
legend("topleft", legend = "A) Wolf Overlap", cex = 2, box.lty = 0, bg = NA, inset = inset)

plot(lynx_overlap, ext = my_bbox, col = my_col, legend = F, pax = pax, mar = mar); plot(DE_districts$geometry[DE_districts$GF != 8], add = T)
legend("topleft", legend = "B) Lynx Overlap", cex = 2, box.lty = 0, bg = NA, inset = inset)

plot(fox_density, breaks = fox_breaks, ext = my_bbox, col = colorRampPalette(my_col)(length(fox_breaks)), legend = F, pax = pax, mar = mar); plot(DE_districts$geometry[DE_districts$GF != 8], add = T)
legend("topleft", legend = "C) Fox density", cex = 2, box.lty = 0, bg = NA, inset = inset)

plot(c(0,2),c(0,1),type = 'n', axes = F,xlab = '', ylab = '', main =  TeX("$\\textbf{Suitabiliy\\ categories}$"), cex.main = 2)
legend("top", legend = plot_levels, fill = my_col, cex = 2, box.lty = 0)


legend_image <- as.raster(matrix(my_col, nrow=1))
text(x=seq(0.1,1.8,l=4), y = 0, labels = seq(minmax_curr[1],minmax_curr[2],l=4) %>% round(2), cex = 2, xpd = T)
rasterImage(legend_image, 0, 0.2, 2,0.1)

text(1, 0.3, TeX("$\\textbf{Fox\\ density}$"), srt = 0, xpd = T, cex =2)
}
...
xtable(area_table,
    caption = "Todays and future suitability area per class in percent for Germany and Europe."
) %>%
    print(file = "./4_output/Plots and tables/suitable areas.txt", caption.placement = getOption("xtable.caption.placement", "top"), include.rownames=FALSE)
...
# Additional Plots
## Countries with Muntjac occurrences
...
point_aoi_inter <- st_intersects(all_occs, AOI, sparse = F) %>%
    apply(., 2, function(x) table(x)["TRUE"])
AOI %<>% add_column(point_aoi_inter)

tm_shape(AOI) +
    tm_fill(col = "point_aoi_inter", palette = "Greens", style = "log10", title = "Occurrences \nper Country") +
    tm_borders(col = "black") +
    tm_compass(type = my_comp_type, show.labels = my_comp_lable, size = my_comp_size, position = my_comp_pos) +
    tm_layout(legend.position = c("left", "top"), legend.text.size = 1.7, legend.title.size = 1.7)
```
