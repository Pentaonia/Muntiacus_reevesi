...
## Self defined functions {.tabset .tabset-pills}
...
```{r}
fuzzySim::percentTestData(5) # in our case: 5 main variables
...
- __distance__ = Tibble with ncol() = length(b) and nrow = nrow(a), Distance of every point in _a_ to the nearest polygon in _b_.
...
__Attention:__ A better solution is in development and might be published in future as a package.
...
- __coltake__ = Object created in the for-loop later on. It is needed if you want only a part of the iterations/chains in your result. It simply is a vector of values $\in$ [1, iterations*chains] for every iteration (of chains) that should be used.
...
## Variables 
### Boundaries: World and Area of interest {.tabset .tabset-pills}
...
### Response (_Muntiacus reevesi_ occurrences) {.tabset .tabset-pills}
#### GBIF
...
water %<>%
    ...
    sf::st_simplify(dTolerance = 100) # The shape has to be simplified because its to big to be handled with adequate speed
...
roads %<>%
    ...
    filter(type != "Ferry Route", type != "Ferry, seasonal") %>% # Important: Filter water ways
    st_transform(crs = 4087)
...
### Distance maps to Urban areas, Water and streets {.tabset .tabset-pills}
#### Step I
...
Note: The distane maps are not available in world size because the eucledean distance calculation is too resource intensive, therefore the distances to roads, waters and urban areas have to be extracted in another way later on.
...
### Future conditions {.tabset .tabset-pills}
#### Future climate
...
#### Resampling
Attention: In my case, the whitebox had a bug in piping the resample function to the shell. Therefore I corrected the function and included it at the beginning.
...
## Input environment {.tabset .tabset-pills}
...
# Test against 1000 Monte Carlo iterations of complete spatial randomness
autocorr$GB_fest <- 
    envelope(
        autocorr$muntgbif_GB_ppp, 
        fun = "Fest", 
        nsim = 1000, 
        funargs = list(r = seq(0, 14000, 500))
    )

autocorr$TW_fest <- 
    envelope(
        autocorr$muntgbif_TW_ppp, 
        fun = "Fest", 
        nsim = 1000, 
        funargs = list(r = seq(0, 14000, 500))
    )
...
# Attention: Overwrites the national analysis!
...
At the end: Export as a table that can be imported into LaTeX.
```{r}
load("./3_ready/model input/PA_WORLD.Rdata")
...
col_result <- col_test_df %>%
    select(
        bio_1, bio_12, road_distance, trees, shrubs, cropland, 
        water_distance, slope, grassland, urban_distance, water
        ) %>%
    multicol_own()
...
owncount(col_result, "VIF", 10, "max") %>%
    mutate(Predictor = case_when(
        factor == "bio_1" ~ "Annual mean temperature",
        factor == "bio_12" ~ "Annual mean precipitation",
        factor == "slope" ~ "Slope",
        factor == "trees" ~ "Trees",
    ...
    xtable(.,
        caption = "Collinearity statistics of the used predictors. Trees = Index combination of all woodland environments, $R^2$ = Proportion of the variation in the response that can be accounted for by the predictors, VIF = $\\frac{1}{R²}$ = Variance inflation factor. Table sorted after VIF. Count = Number of significant corellated variables + Intercept.",
        label = "tab:collinearity"
        ) %>%
    print(
        file = "./4_output/Plots and tables/Collinearity.tex", 
...
Adapt the formula further down to your needs.
...
- __test_stat.RData__ = RData document that includes the calculated test statistics. All objects are lists of lenght == cv.runs.
    ...
    - auc.cv = Area under the curve value per model $\in$ [0,1].
    - tss.thresh = Threshold where TSS $\in$ [0,1] is highest.
    - tss = True skill statistics test value $\in$ [0,1].
    - acceptance = Is the model accepted $\in$ {TRUE, FALSE}.
    - accurr = Accurracy of the model $\in$ [0,1].
    - convergence = Internal convergence check $\in$ [0,1], has to be 1 .
    - tree_vars_imp = Variable importance, mean and standard deviation per used environmental variable per CV-run. List of arrays with list lenght == cv.runs and array dimensions = {rows, cols, depth}.
      - rows = length(most.important)
      - cols = 2, mean and sd
      - depth = cv.runs 
    - bri = Brier Score $\in$ [0,1].
...
for (i in 1:cv.runs) {
    ...
    # save model and test results every 2. run in case of PC crashing when calculating big models
    if (i %% 2 == 0) {
...
tree_vars_imp_mean_EU <- 
    array(
        unlist(tree_vars_imp_EU), 
        dim = c(nrow(tree_vars_imp_EU[[1]]), 2, length(tree_vars_imp_EU))
    ) %>%
    apply(1:2, mean) %>%
    as_tibble() %>%
    mutate(
...
my_dataframe_EU <- 
    reshape2::melt(tree_vars_imp_EU, level = 2) %>% 
    mutate(name = rep(rownames(tree_vars_imp_EU[[1]]), max(L2) * 2))
...
tree_vars_imp_mean_WORLD <- 
    array(
        unlist(tree_vars_imp_WORLD), 
        dim = c(nrow(tree_vars_imp_WORLD[[1]]), 2, length(tree_vars_imp_WORLD))
    ) %>%
    apply(1:2, mean) %>%
    as_tibble() %>%
    mutate(
...
# Convert matrix to data frame for use in ggplot2
my_dataframe_WORLD <- 
    reshape2::melt(tree_vars_imp_WORLD, level = 2) %>% 
    mutate(name = rep(rownames(tree_vars_imp_WORLD[[1]]), max(L2) * 2))
...
plots <- 
    list(
        tree_vars_imp_plot_EU, tree_vars_imp_plot_EU_adv, 
        tree_vars_imp_plot_WORLD, tree_vars_imp_plot_WORLD_adv, tree_vars_imp_plot_tog
    )
...
## Test statistics
One model run per intex.
```{r}
index <- 1
evaluation.cv[index] # Can be used in the ROCR internal statistic functions
...
test_stat3_sd <- 
    c(
        test_stat %>% apply(., 2,sd), 
        test_stat2 %>% apply(., 2,sd)
    ) %>% 
    round(3)
...
rownames(test_stat5) <- c("Minimum threshold", "Europe", "World")
colnames(test_stat5) <- colnames(test_stat2)

test_stat5 %<>% t()

xtable(test_stat5,
        caption = "Minimum thresholds for for model acceptance and mean test statistics of the 25 cross-validation runs of both the conservative (Europe) and less conservative (World) model runs. AUC = Area under the receiver operator curve, Accuracy = Accurracy of the models, TSS = True skill statistics, TSS Thresh = Threshold of suitability where Accuracy maximal, Accepted = Acceptance of the model run (1 = Accepted, 0 = Not accepted), Divergent = Check whether the model internal algorithm discovers divergency and BRI = Brier score. Values including  standard deviation calculated across the cross validation runs.",
...
EU-wide plots of the suitability for today, future prediction, and the standard deviations between all model runs
...
#### Base - Germany
Germany-wide plots of the suitability for today, future prediction, and the standard deviations between all model runs
```{r}
pdf(
...
ifel(wolf_work2 %in% seq(100, 900, 1), 10, 0) %>% 
    writeRaster("./2_work/predators/wolf_work3.tif", overwrite = T)
...
# Calculation
ifel(lynx_work2 %in% c(seq(10, 30, 1), seq(450, 500, 1)), 10, 0) %>% 
    writeRaster("./2_work/predators/lynx_work3.tif", overwrite = T)
...
fox_density %<>% 
    crop(AOI[AOI$Name == "DEU", ]) %>% 
    mask(AOI[AOI$Name == "DEU", ], filename = "./4_output/Predators/Fox_density.tif", overwrite = T)
```
...
# Area calculations {.tabset .tabset-pills}
## Europe
```{r}
## Today
...
c(
    ...
    "Lynx_Cover_Germany" = Lynx_cover_area[2, 3] - 0.01 # minus 1 % due to large digitalization fragments within the map
)
```
...
# paste necessary parts of the subtitle and specific settings into the graph
for (i in 1:5) {
    g_plot1[[i]] <- g_plot1[[i]] + 
