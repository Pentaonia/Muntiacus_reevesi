...
List the first n results of a data frame sorted by a column. Interesting for seeing the most collinear values late on.
...
Attention: Here is implemented directly the rule of thumb after Huberty (1994) reviewed in [Fielding, A. H., & Bell, J. F. (1997)](https://doi.org/10.1017/S0376892997000088) for splitting into train and test data.
...
I do not know when it is fixed, therefore I included the fixed function here.
...
That means, if n = 1 than the occurrence has one neighbour, therefore 0.5/1 = 0.5 and bot occurrences are weighted 0.5.
...
#### Step I
The fist step was the rasterization of the urban areas, waters and streets in QGIS. Export them as "WORLD_roads_raster.tif", "WORLD_urban_raster.tif" and "WORLD_water_raster.tif" with CRS = EPSG4087. R works too ineficient on large rasters. You can also try the ´fasterize´ function but its not working with MULTILINESTRING at the moment or use the "RQGIS"-Package.
...
Note: The distane maps are not available in world size, therefore the distances to roads, waters and urban areas have to be extracted on an other way later on.
...
For prediction only Europe is needed.
```{r}
